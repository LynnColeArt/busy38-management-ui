#!/usr/bin/env python3
"""Run a live management-plane smoke test against sibling Busy sources."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the Busy38 management-plane smoke test")
...
def main() -> int:
    ...
    read_token = f"viewer-{secrets.token_hex(8)}"
    pairing_secret = f"pairing-{secrets.token_hex(16)}"

    temp_root = Path(tempfile.mkdtemp(prefix="busy38-management-smoke-"))
    runtime_root = temp_root / "runtime"
    runtime_root.mkdir(parents=True, exist_ok=True)
    log_path = temp_root / "management-plane-smoke.log"
...
bridge_service._GM_TICKETS["smoke-ticket-1"] = {{
    ...
    "title": "Management smoke seeded room",
    "state": "queued",
    "team_id": "qa",
    "assigned_to": "carlo",
...
"""
    ...
    env.update(
        {
            ...
            "BUSY38_MOBILE_DISCOVERY_LABEL": "Management Smoke",
    ...
    try:
        ...
        status, content_type, html = request_text(f"{base_url}/")
        assert_equal(status, 200, "root path should succeed")
        assert_true("text/html" in content_type, "root path should serve HTML")
        assert_true("busy38-management-api-base" in html, "root HTML should include API base meta tag")

        status, _, admin_html = request_text(f"{base_url}/admin")
        assert_equal(status, 200, "SPA fallback path should succeed")
        assert_true("busy38-management-api-base" in admin_html, "SPA fallback should serve the app shell")

        read_headers = {"Authorization": f"Bearer {read_token}"}
        admin_headers = {"Authorization": f"Bearer {admin_token}"}

        status, appearance = request_json("GET", f"{base_url}/api/appearance", headers=read_headers)
        assert_equal(status, 200, "viewer appearance read should succeed")
        assert_equal(appearance["appearance_preferences"]["override_enabled"], False, "default override should be false")

        status, _ = request_json(
            "PATCH",
            f"{base_url}/api/appearance",
            headers=read_headers,
            body={"override_enabled": True, "shared_theme_mode": "dark"},
        )
        assert_equal(status, 403, "viewer appearance write should be forbidden")

        status, updated_appearance = request_json(
            "PATCH",
            f"{base_url}/api/appearance",
        ...
        assert_equal(status, 200, "admin appearance write should succeed")
        prefs = updated_appearance["appearance_preferences"]
        ...
        assert_equal(descriptor["display_label"], "Management Smoke", "discovery label should match override")
        assert_true("local_network_code" in descriptor["bootstrap_methods"], "discovery should advertise local-network bootstrap")
        ...
        print(f"management-plane smoke passed against {base_url}")
